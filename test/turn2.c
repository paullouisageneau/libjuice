...
int test_turn_udp_preferred(void) {
	...
	// Scenario 1 "TCP": TCP TURN only.
	//   No TURN UDP server, so TURN TCP is not delayed and is attempted right away.
	//   Expected: one TCP relay gathered, TCP transport selected.
	// Scenario 2 "UDP+TCP": UDP TURN + TCP TURN.
	//   TURN TCP is delayed behind TURN UDP; the UDP relay succeeds within the head start, so the
	//   TURN TCP connection is skipped (and the relay candidate gets a lower priority anyway).
	...
	static const struct {
		...
		{ "TCP",     false, JUICE_TURN_TRANSPORT_TCP },
		{ "UDP+TCP", true,  JUICE_TURN_TRANSPORT_UDP },
	};
