...
void run_passive_ice_tcp(socket_t server_socket) {
	...
	for (int i = 0; i < 2;) {
		if ((n = tcp_ice_read(client_socket, server_buffer, ICE_TCP_SERVER_BUFFER_SIZE, &ice_tcp_len)) == -1) {
			return;
		} else if (n == 0) {
			continue;
		}
		...
		if (tcp_ice_write(client_socket, server_buffer, n) == -1) {
			return;
		}
...
int test_tcp() {
	...
	config.ice_tcp_mode = JUICE_ICE_TCP_MODE_ACTIVE;
	...
	snprintf(sdp, JUICE_MAX_SDP_STRING_LEN,
	         "a=ice-ufrag:ufrag\r\n"
	         "a=ice-pwd:%s\r\n"
	         "a=candidate:1 1 TCP 2122316799 127.0.0.1 %d typ host\r\n",
	         ICE_PWD, ice_tcp_server_port);
