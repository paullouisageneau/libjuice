...
int agent_set_remote_description(juice_agent_t *agent, const char *sdp) {
	...
	int ret = ice_parse_sdp(sdp, &remote, agent->ice_tcp_mode == JUICE_ICE_TCP_MODE_ACTIVE);
	if (ret < 0) {
		switch (ret) {
...
int agent_add_remote_candidate(juice_agent_t *agent, const char *sdp) {
	...
	if (ret < 0) {
		...
		if (is_tcp && agent->ice_tcp_mode != JUICE_ICE_TCP_MODE_ACTIVE) {
		...
		for (int i = 0; i < agent->remote.candidates_count; ++i) {
			if (strcmp(agent->remote.candidates[i].transport, "TCP") == 0) {
				JLOG_DEBUG("Only one ICE-TCP remote candidate is supported ignoring Candidate: %s", sdp);
				conn_unlock(agent);
				return JUICE_ERR_IGNORED;
			}
		}
