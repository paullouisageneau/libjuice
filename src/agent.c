...
void agent_add_ice_tcp_local_candidate(juice_agent_t *agent, addr_record_t *record) {
	...
	if (ice_create_local_candidate(ICE_CANDIDATE_TYPE_HOST, 1, agent->local.candidates_count, record, &candidate, true)) {
		return;
	}
...
void agent_stun_entry_for_candidate_pair(juice_agent_t *agent, ice_candidate_pair_t *pair, agent_stun_entry_t *relay_entry) {
...
void agent_tcp_conn_connected(juice_agent_t *agent) {
	for (int i = 0; i < agent->candidate_pairs_count; ++i) {
		...
		if (strcmp(pair->remote->transport, "TCP") == 0) {
			agent_stun_entry_for_candidate_pair(agent, pair, NULL);
...
int agent_add_local_reflexive_candidate(juice_agent_t *agent, ice_candidate_type_t type,
	...
	ice_candidate_t candidate;
	if (ice_create_local_candidate(type, 1, agent->local.candidates_count, record, &candidate, false)) {
...
int agent_add_candidate_pair(juice_agent_t *agent, ice_candidate_t *local, // local may be NULL
	...
	bool is_tcp = strcmp(remote->transport, "TCP") == 0;
	if (is_tcp) {
		if (agent->ice_tcp_mode != JUICE_ICE_TCP_MODE_ACTIVE) {
		...
		if (remote->tcp_type != ICE_CANDIDATE_TCP_TYPE_PASSIVE && remote->tcp_type != ICE_CANDIDATE_TCP_TYPE_SIMULTANEOUS_OPEN) {
	...
	*pos = pair;
	...
	if (!is_tcp) {
		agent_stun_entry_for_candidate_pair(agent, pos, relay_entry);
	}
