...
void conn_poll_process_pfd(juice_agent_t *agent, conn_impl_t *conn_impl, struct pollfd *pfd, bool is_tcp) {
...
int conn_poll_process(conn_registry_t *registry, pfds_record_t *pfds) {
	...
	for (int j = 0; j < registry->agents_size; ++j) {
		...
		conn_poll_process_pfd(agent, conn_impl, &pfds->pfds[i], false);
		i++;
		...
		conn_poll_process_pfd(agent, conn_impl, &pfds->pfds[i], true);
		i++;
	}
