...
int conn_poll_prepare(conn_registry_t *registry, pfds_record_t *pfds, timestamp_t *next_timestamp) {
	...
	*next_timestamp = now + 60000;
	...
	for (nfds_t i = 1; i < pfds->size; i += 2) {
		struct pollfd *pfd = pfds->pfds + i;
		struct pollfd *pfd_tcp = pfds->pfds + i + 1;

		juice_agent_t *agent = registry->agents[(i - 1) / 2];
		if (!agent) {
			pfd_tcp->fd = pfd->fd = INVALID_SOCKET;
			pfd_tcp->events = pfd->events = 0;
			continue;
		}
		...
		pfd->fd = conn_impl->udp_sock;
		pfd->events = POLLIN;

		pfd_tcp->fd = conn_impl->tcp_sock;
...
int conn_poll_process(conn_registry_t *registry, pfds_record_t *pfds) {
	...
	for (nfds_t i = 1; i < pfds->size; ++i) {
		...
		juice_agent_t *agent = registry->agents[agent_index];
		if (!agent)
			continue;
		...
		if (conn_impl->tcp_sock != pfd->fd && conn_impl->udp_sock != pfd->fd) {
			continue;
		}
		...
		if (pfd->revents & POLLIN) {
			...
			while (left--) {
				if (is_tcp) {
					if ((ret = tcp_ice_read(conn_impl->tcp_sock, buffer, BUFFER_SIZE)) <= 0)  {
						break;
					}
					memcpy(&src, &conn_impl->tcp_dst, sizeof(conn_impl->tcp_dst));
				} else {
					if ((ret = conn_poll_recv_udp(conn_impl->udp_sock, buffer, BUFFER_SIZE, &src)) <= 0)  {
						break;
					}
				}
...
int conn_poll_send(juice_agent_t *agent, const addr_record_t *dst, const char *data, size_t size,
	...
	if (dst->socktype == SOCK_STREAM) {
		...
		ret = tcp_send(conn_impl->tcp_sock, data, size);
	} else {
	...
	if (ret < 0) {
		...
		else if (sockerrno == SEENOTCONN) // TCP not connected yet
			ret = 0;
		else
